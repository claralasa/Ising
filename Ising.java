...
/*
  open(unit=2,file='SAMPLES',form='formatted',
 $     status='unknown')
  open(unit=1,file='PHOTO',form='formatted',
 $     status='unknown')*/
...
//
           ...
           // In order to map the energy from ien, a 4 is needed to be added.
           if (RandomGen.nextFloat() < w[ien+4]) {
...
//    Perform the averages only if we have finished the equilibration
     if (mcs > nequil) {
        ...
        if (mcs % ndelta == 0) {
           ...
           mag = invsize * m;
           System.out.format("%6d\t %8.6f\t %10.6f", nsample, invsize*energy, mag);
           acc_mag = acc_mag + mag;
...
// $k_{\rm B} T \chi = \langle M^{2} \rangle - \langle M \rangle^{2}
  ...
  /*write(6,9100) L, red_temp, ave_mag, ji,
 $             mcsmax, ndelta, nequil
9100 format(i3,f8.4,2x,'M: ',f10.4,1x,'Suscep: ',f10.4,5x,3i7)
  
// Print a picture with the final configuration of spins
  for(int i = 0; i<L; i++){
     write(1,'(80i1)') (lattice[i][j]+1,j=1,L)
         }*/
}
